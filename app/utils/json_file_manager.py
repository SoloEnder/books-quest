        ...
        **msg_infos,
        ...
        if catch_error:
            try:
                ...
                self.logger.error(
                    f"Unable to write JSON in file {filepath} : permission denied !"
                )
        ...
        **msg_infos,
        ...
        if catch_error:
            try:
                ...
                self.logger.error(
                    f"Unable to read JSON in file {filepath} : permission denied !"
                )
                self._raise_msg(**msg_infos)

            except FileNotFoundError:
                self.logger.error(
                    f"Unable to read JSON in file {filepath} : file not found !"
                )
                self._raise_msg(**msg_infos)
...
class JsonFileManager:
    ...
    @catch_writing_error
    def write_json(
        self, filepath: str | pathlib.Path, data, catch_error: bool = True, **msg_infos
    ):
    ...
    @catch_reading_error
    def read_json(
        self, filepath: str | pathlib.Path, catch_error: bool = True, **msg_infos
    ):
