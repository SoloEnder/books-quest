...
def run(patch_path: str, installation_path: str):
    check_patch(patch_path, installation_path)
    patch_infos = getpinfos(patch_path)
    upgrade_to_semantic = patch_infos["upgrade_to"]["semantic"]
    target_version_semantic = patch_infos["target_version"]["semantic"]
    extracted_patch_path = os.path.join(
        installation_path, f"patch_{upgrade_to_semantic}"
    )
    backup_folder = os.path.join(installation_path, f"backup_{target_version_semantic}")
    utils.check_and_make_folder(backup_folder)
    utils.check_and_make_folder(extracted_patch_path)
    extract_patch(patch_path, extracted_patch_path)
    instructions = get_patch_instruction(extracted_patch_path)
...
def move_item(
    ...
    undo.append(
        {"type": "remove", "path": os.path.join(old_to, os.path.basename(from_))}
    )
...
def get_patch_instruction(patch_path: str, extracted: bool = True):
    if extracted:
        return utils.read_json(os.path.join(patch_path, "instructions.json"))

    else:
        with zipfile.ZipFile(patch_path, "r") as zpf:
            return json.loads(zpf.read("instructions.json"))
...
def getpinfos(patch_path: str) -> dict:
    ...
    """
    with zipfile.ZipFile(patch_path, "r") as zpf:
        with zpf.open("infos.json", "r") as f:
            return json.load(f)


def extract_patch(patch_path: str, dest_dir: str):
    with zipfile.ZipFile(patch_path, "r") as zpf:
...
def check_patch(patch_path: str, installation_path: str):
    with zipfile.ZipFile(patch_path, "r") as zpf:
        zip_content = zpf.namelist()
        excepted_content = ("content/", "infos.json", "instructions.json")
        missings_elements = []
        for excepted in excepted_content:
            if excepted not in zip_content:
                missings_elements.append(excepted)

        if missings_elements:
            raise MissingsElementsError(("content", "infos.json", "patch_ins.json"))

        patch_infos = getpinfos(patch_path)
        excepted_keys = {
            "target_version": {"readable"},
            "upgrade_to": "",
            "creation_date": "",
            "format_version": "",
        }
        if not patch_infos.keys() == excepted_keys.keys():
...
class MissingsElementsError(utils.MyBaseException):
...
class InvalidPatchInfos(utils.MyBaseException):
    def __init__(self):
        super().__init__()
        self.msg = "The information declared by the patch are not in a valid formet !"


class UnsupportedPatchVersion(utils.MyBaseException):
...
class UntargetedInstallationError(utils.MyBaseException):
...
class InvalidUpdateInstructions(utils.MyBaseException):
    def __init__(self):
        super().__init__()
        self.msg = "The update instructions of the patch file are not valid !"


class DownGradeError(utils.MyBaseException):
