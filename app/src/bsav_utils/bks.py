...
def create_bks_file(bks_data: tuple[dict, list], covers_dir: str, dest_dir):
    """
    ...
    dest_dir: the directory where to put the result save file
    """
    utils.write_json(os.path.join(dest_dir, "infos.json"), bks_data[0])
    utils.write_json(os.path.join(dest_dir, "books.json"), bks_data[1])
    with zipfile.ZipFile(os.path.join(dest_dir, "books.bks"), "w") as zpf:
        zpf.write("infos.json")
        zpf.write("books.json")
        zpf.mkdir("covers")
...
def open_bks_file(save_path: str, dest_dir: str) -> dict:
    """
    ...
    utils.check_save(
        ...
        ("books.json", "infos.json", "covers.json"),
    )
