...
def create_bsav(
    ...
    bks.create_bks_file(bks_data, books_cover_dir, dest_dir)
    bbs_data = bbs.create_bbs_data(shelves_data, "1.0.0")
    bks.create_bks_file(bbs_data, shelves_cover_dir, dest_dir)
    bsav_infos = create_bsav_infos(program_version)
...
def create_bsav_infos(program_version: str):
    """
...
def open_bsav_file(save_path, dest_dir: str, extrac_sub: bool = True) -> dict | None:
    """
    ...
    utils.check_save(
        ...
        SUPPORTED_VERSIONS,
        ["BSAV"],
        ("shelves.bbs", "books.bks", "infos.json"),
    )
