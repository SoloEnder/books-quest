...
mrfh = my_logging_stuff.MyRotatingFileHandler(
    os.path.join(paths.APP_PATH, "logs", "main.log")
)
...
try:
    splash_screen = QtWidgets.QSplashScreen(
        QtGui.QPixmap(
            os.path.join(paths.APP_PATH, "assets", "splashscreen", "splashscreen.png")
        )
    )
    splash_screen.showMessage(
        "Initialising Books Quest...",
